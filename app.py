...
def is_safe_url(url):
    try:
        ...
        if ip_obj.is_loopback: return False, "Yerel ağa (localhost) tarama yapılamaz."
        if ip_obj.is_private: return False, "Özel/İç ağ (Private IP) adreslerine tarama yapılamaz."
        if ip_obj.is_link_local: return False, "Bulut metadata adreslerine erişim yasaktır."
...
def start_scan():
    ...
    if not safe: return jsonify({"error": err_msg}), 400
    if not modules: return jsonify({"error": "En az bir modül seçin"}), 400
...
def get_status(scan_id):
    q = scan_queues.get(scan_id)
    if not q: return jsonify({"status": "error", "msg": "Tarama bulunamadı"})
    msgs = []
    while True:
    ...
    if is_done: scan_queues.pop(scan_id, None)
...
def get_news():
    ...
    try:
        ...
        for item in items[:NEWS_COUNT]:
            ...
            if not image and link and link != "#": image = _og_image(_fetch_url(link, timeout=6))
        ...
        if _news_cache["data"]: return jsonify({"articles": _news_cache["data"]})
